...
class ZionWallet:
    ...
    def __init__(self, wallet_file: str = "zion_wallet.json", password: Optional[str] = None):
        """
        Initialize ZionWallet with mandatory encryption
        
        Args:
            wallet_file: Path to wallet file
            password: Password for encryption (REQUIRED for new wallets)
        """
        ...
        # If new wallet (no addresses), require password immediately
        if not self.addresses:
            if not password:
                raise ValueError(
                    "SECURITY ERROR: Password required for new wallet! "
                    "Call: ZionWallet('wallet.json', password='your_password')"
                )
            # Set encryption key from password
            self.set_password(password)
            # Now create default address with encryption enabled
    ...
    def set_password(self, password: str):
        ...
        try:
            ...
            # SECURITY FIX REAL-CRIT-001: Encryption is MANDATORY
            raise ImportError(
                "CRITICAL SECURITY ERROR: Cryptography library not available!\n"
                "Cannot create wallet without encryption.\n"
                "Install: pip install cryptography\n"
                f"Error: {e}"
            )

    def _encrypt_private_key(self, private_key: str) -> str:
        """Encrypt private key - MANDATORY ENCRYPTION
        
        SECURITY FIX REAL-CRIT-001:
        - Encryption is now REQUIRED, no fallback to plain text
        - Raises exception if encryption key not available
        - Prevents creation of unencrypted wallets
        """
        if not self.encryption_key:
            raise ValueError(
                "CRITICAL SECURITY ERROR: Encryption key not set!\n"
                "Cannot store private keys without encryption.\n"
                "Wallet creation failed - this is a security feature."
            )
        ...
        try:
            ...
            # SECURITY: Do NOT fallback to plain text on error
            raise ValueError(
                f"CRITICAL SECURITY ERROR: Private key encryption failed!\n"
                f"Error: {e}\n"
                f"Wallet NOT created - this protects your funds."
            )

    def _decrypt_private_key(self, encrypted_key: str, encrypted: bool = False) -> str:
        """Decrypt private key - SECURE DECRYPTION ONLY
        
        SECURITY FIX REAL-CRIT-001:
        - Only decrypts if encryption_key is set
        - Raises exception on decryption failure
        - No fallback to returning encrypted value
        """
        if not encrypted:
            raise ValueError(
                "SECURITY ERROR: Attempting to decrypt non-encrypted key!\n"
                "This should never happen. Wallet may be corrupted."
            )
        
        if not self.encryption_key:
            raise ValueError(
                "SECURITY ERROR: Cannot decrypt - encryption key not set!\n"
        ...
        try:
            ...
            # SECURITY: Do NOT return encrypted value on error
            raise ValueError(
                f"SECURITY ERROR: Private key decryption failed!\n"
                f"Error: {e}\n"
                f"Possible causes: wrong password, corrupted wallet, or tampering."
            )
    ...
    def save_wallet(self):
        """Public method to save wallet (calls _save_wallet)"""
        self._save_wallet()
