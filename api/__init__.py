...
from security_validators import validate_address, sanitize_input, sanitize_html
...
async def rate_limit_middleware(request: Request, call_next):
    ...
    SECURITY: Limits requests to 60 per minute per IP
    """
...
async def get_address_balance(address: str):
    ...
    SECURITY: Validates address format before query
    """
...
if __name__ == "__main__":
    ...
    # SECURITY FIX: Default to localhost, allow 0.0.0.0 only via environment variable
    # This prevents accidental exposure to the internet
    host = os.getenv('ZION_API_HOST', '127.0.0.1')
    port = int(os.getenv('ZION_API_PORT', '8000'))
    
    if host == '0.0.0.0':
        print("⚠️  WARNING: Binding to 0.0.0.0 (all interfaces)")
        print("⚠️  This exposes the API to your network and potentially the internet!")
        print("⚠️  Make sure you have proper firewall rules in place.")
        print("⚠️  For local development, use: ZION_API_HOST=127.0.0.1")
