...
SECURITY NOTE: KeyPair uses memory-safe private key handling.
"""
...
class KeyPair:
    """
    Secure KeyPair storage with protection against logging/debugging exposure.
    
    SECURITY FEATURES:
    - __repr__ redacted to prevent accidental logging
    - Private key stored separately (not in dataclass fields)
    - Memory-safe access through methods only
    """
    public_key_hex: str
    _private_key_hex: str = field(repr=False, default="")
    
    def __repr__(self) -> str:
        """Prevent accidental private key exposure in logs/debugging"""
        return f"KeyPair(public_key={self.public_key_hex[:16]}...[REDACTED], address={self.address()})"
    
    def __str__(self) -> str:
        """User-friendly representation without private key"""
        return f"KeyPair<{self.address()}>"
    
    @property
    def private_key_hex(self) -> str:
        """
        Access private key (use with caution!)
        
        WARNING: Only use this when absolutely necessary for signing.
        Never log, print, or expose this value.
        """
        if not self._private_key_hex:
            raise ValueError("SECURITY ERROR: Private key not available or has been cleared!")
        return self._private_key_hex
    
    def clear_private_key(self):
        """Clear private key from memory (for hot/cold wallet separation)"""
...
def generate_keypair() -> KeyPair:
    ...
    return KeyPair(
        ...
        _private_key_hex=sk.to_string().hex()
    )
