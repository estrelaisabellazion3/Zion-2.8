...
const ZionAIBackend: React.FC = () => {
  ...
  // Fetch AI backend data from new orchestrator API
  const fetchAIData = async () => {
    try {
      ...
      if (data.success) {
        // Map orchestrator data to old format for compatibility
        const mappedData: AIBackendData = {
          ai_systems: {},
          system_status: {},
          performance_metrics: {
            total_requests: 0,
        ...
        if (data.data.components) {
          data.data.components.forEach((comp: string) => {
            mappedData.ai_systems[comp] = {
              name: comp,
              status: 'active',
            ...
            mappedData.system_status[comp] = true;
          });
        }
